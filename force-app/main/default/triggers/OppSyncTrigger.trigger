/************************************************************************************************************************
* Name: OppSyncTrigger 
* Description: OppSyncTrigger  is the part of custom quote sync unmanage package.
* Changes: 0.1 (Pooja Deokar (TCS))
* Description:Added the Petchem record type condition to avoid the oli override issue with same values.
************************************************************************************************************************/
...
trigger OppSyncTrigger on Opportunity (after update) {
    ...
    for (Opportunity opp : trigger.new) {
        //PCRM(24Aug2020) : Added record type condition to avoid the fist time quote sync condition. To resolve the issue #1448894(2nd Point)
        if (opp.SyncedQuoteId != trigger.oldMap.get(opp.Id).SyncedQuoteId && opp.RecordTypeId != opportunityRecordTypeId) {
