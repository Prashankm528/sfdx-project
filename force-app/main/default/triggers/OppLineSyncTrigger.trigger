trigger OppLineSyncTrigger on OpportunityLineItem (before insert, after insert, after update) {
    system.debug('IN OppLineSyncTrigger');
    ...
    String oliQuery = 'select Id, OpportunityId, PricebookEntryId, PricebookEntry.Product2.Id, Discount, ServiceDate, SortOrder' + oliFields + ' from OpportunityLineItem where Id in (' + oliIds + ') order by OpportunityId, SortOrder ASC';
    ...
    if (quoteIds != '') {
        ...
        String qliQuery = 'select Id, QuoteId, PricebookEntryId, OpportunityLineItemId, description, PricebookEntry.Product2.Id, Discount, ServiceDate, SortOrder' + qliFields + ' from QuoteLineItem where QuoteId in (' + quoteIds + ') order by QuoteId, SortOrder ASC';   
